...
> **Пропущенные слоты**: если слот уже наступил (например, GitHub Actions запустился
> с задержкой из-за очереди долгих job), а по нему ещё нет записи в `data/state.json`,
> он навёрстывается при следующем запуске (в пределах `MISSED_SLOT_MAX_MIN`, по умолчанию 180 мин).
> Для ручной публикации всех пропущенных слотов дня: `FORCE_MISSED_POSTS=1`.
